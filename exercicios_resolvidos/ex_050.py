...
def sorteia_numeros(qtd_numeros: int, inicio: int, fim: int) -> list[int]:
...
def qtd_nums_acima_5(nums: list[int]) -> int:
    qtd = 0
    for num in nums:
        if num > 5:
            qtd += 1
...
def qtd_nums_divisiveis_3(nums: list[int]) -> int:
    qtd = 0
    for num in nums:
        if num % 3 == 0:
            qtd += 1
...
def exibir_dados(nums: list[int], acima_cinco: int, divisiveis_tres: int):
    print(
