...
def calcula_idade(ano_nascimento: int) -> int:
...
def situacao_alistamento(idade: int) -> str:
    if idade < 18:
        ...
        passou = idade - 18
        resultado = (
            f"Já se passaram {passou} anos para se alistar, se aliste imediatamente!\n"
        )

    else:
        resultado = "Você esta na idade ideal para se alistar!\n"

    return resultado


def exibir_dados(ano_nascimento: int, idade: int, situacao: str):
    print("\n---- Dados ----\n")
    print(
        f"Ano nascimento: {ano_nascimento}\nIdade: {idade} anos\nSituação: {situacao}\n"
    )
