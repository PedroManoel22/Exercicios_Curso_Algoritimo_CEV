...
def valida_valor_compras() -> float:
    while True:
        try:
...
def calcula_desconto(sexo: str, valor_compras: float):
    if sexo == "H":
...
def exibir_dados(nome: str, sexo: str, valor_compras: float, valor_desconto: float):
    if sexo == "H":
        desconto = 5
