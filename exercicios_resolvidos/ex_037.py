...
def valida_salario_atual() -> float:
    while True:
        try:
...
def valida_anos_trabalhando() -> int:
    while True:
        try:
...
def calcula_reajuste_salarial(
    salario: float, genero: str, anos: int
) -> tuple[float, int]:
    reajuste = salario
    porcentagem = 0
...
def exibir_dados(
    salario: float, genero: str, anos: int, reajuste: float, porcentagem: int
):
    print("\n---- Dados ----\n")
    print(
