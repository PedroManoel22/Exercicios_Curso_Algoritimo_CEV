...
def calcula_pontos(horas: int) -> int:
    if horas < 10:
        pontos = horas * 2
...
def converte_pontos_para_dinheiro(pontos: int) -> float:
    premio = pontos * 0.05
    return premio


def exibir_dados(horas: int, pontos: int, premio: float):
    print("\n---- Resultado ----\n")
    print(
        f"Horas de atividade física: {horas}h\nPontos: {pontos}\nPremio: R${premio}\n"
    )
