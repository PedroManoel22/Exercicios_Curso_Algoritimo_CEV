...
def valida_primeiro_segmento() -> float:
    while True:
        try:
...
def valida_segundo_segmento() -> float:
    while True:
        try:
...
def valida_terceiro_segmento() -> float:
    while True:
        try:
...
def forma_triangulo(seg1: float, seg2: float, seg3: float) -> bool:
    regras = 0
    if seg1 < seg2 + seg3:
        regras += 1
...
def tipo_triangulo(seg1: float, seg2: float, seg3: float) -> str:
...
def exibir_dados(
    seg1: float, seg2: float, seg3: float, e_triangulo: bool, tipo: str = "nada"
):
    if tipo == "nada":
        if e_triangulo:
